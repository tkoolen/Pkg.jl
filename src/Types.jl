...
function clone_or_cp_registries(ctx::Context, regs::Vector{RegistrySpec}, depot::String)
    ...
    for reg in regs
        # TODO: Don't hardcode General=23338594-aafe-5451-b93e-139f81909106 like this
        if reg.uuid == UUID("23338594-aafe-5451-b93e-139f81909106") || reg.name == "General"
            reg.url = "https://github.com/JuliaRegistries/General.git"
        end
        ...
        elseif reg.url !== nothing # clone from url
            ...
            cmderror("no path or url specified for registry")
        end
...
# find a registry in a DEPOT
function find_registry_locations!(reg::Vector{RegistrySpec}, throw=false)
    # Find the location of all the input registries
    all_regs = collect_registries(; clone_default=false)
    for reg in regs
        if reg.path === nothing
            if reg.uuid !== nothing # search by uuid
                r = findfirst(x -> x.uuid == reg.uuid, all_regs)
                if r === nothing
                    str = "no registry found with uuid $(repr(reg.uuid))"
                    throw ? cmderror(str) : @info(str)
                else
                    reg.path = all_regs[r].path
                end
            elseif reg.name !== nothing # search by name, if unique
                r = findall(x -> x.name == reg.name, all_regs)
                if isempty(r)
                    str = "no registry found with uuid $(repr(reg.name))"
                    throw ? cmderror(str) : @info(str)
                else
                    if !all(x -> x.uuid == all_regs[r[1]].uuid, all_regs[r]) # complain that name was not enough
                        cmderror("multiple registries with name `$(rspec.name)`, please specify with uuid.")
                    end
                end
                reg.path = all_regs[r[1]].path
            end
        end
    end
    return regs
            ...
            if needle.uuid !== nothing
                ...
                if needle.name == candidate.name
                    ...
                    if !all(r -> r.uuid == first(named_regs).uuid, named_regs)
                        cmderror("multiple registries with name `$(needle.name)`, please specify with uuid.")
                    end
                    push!(output, candidate)
                    found = true
                end
